import { StateModel, Test, TestGroup, Snapshot } from 'luis';
...
function addTests(group: TestGroup, testData: jest.AggregatedResult, snapshots: Snapshots) {
  // add info
    ...
    for (let test of suite.testResults) {
      ...
      if (testPath == path) {
        ...
        if (test.failureMessages.length) {
          // disect equality
          let message = test.failureMessages[0];
...
export function createBridge(
  ...
  glob.describe = function(name: string, impl: Impl) {
    ...
    addTests(group, testData, snapshots);
